  ...
  <form method="POST" action="{{ route('materials.update', $material) }}" class="bg-white p-3 rounded">
    ...
    <div class="mb-3">
      ...
      <select class="form-select" name="is_active" required>
        <option value="1" @selected((string)old('is_active', $material->is_active) === '1')>รับซื้อ</option>
        <option value="0" @selected((string)old('is_active', $material->is_active) === '0')>ปิดการรับซื้อ</option>
      </select>
    </div>
